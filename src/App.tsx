        ...
        >
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5 relative">
            ...
            <div className="flex items-center gap-5">
              <div className="font-serif italic text-2xl tracking-[2px] text-nocturne-accent select-none uppercase">
                NỘI DUNG
              </div>
              <span className="font-sans text-[9px] uppercase tracking-[2px] text-nocturne-dim mt-0.5 border-l border-white/10 pl-4">
              ...
              <motion.button
                whileHover={{ scale: 1.05, backgroundColor: "rgba(225, 169, 95, 0.1)" }}
                whileTap={{ scale: 0.95 }}
                onClick={() => fileInputRef.current?.click()}
                disabled={isUploading}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-nocturne-dim hover:text-nocturne-accent transition-all group"
              >
                {isUploading ? (
                  <Loader2 className="w-3 h-3 animate-spin" />
                ) : (
                  <FileUp className="w-3 h-3 group-hover:-translate-y-0.5 transition-transform" />
                )}
                <span className="text-[8px] uppercase tracking-[1.5px] font-bold">TẢI TÀI LIỆU</span>
              </motion.button>
            </div>
